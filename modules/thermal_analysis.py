...
def analyze_thermal_conditions(
    ...
    # Assess bearing temperature
    def assess_temp(temp: float) -> Dict:
        if temp > 100:
    ...
    if temp_de > 80:
        recommendations.append(
            f"⚠️ Bearing DE temperature {temp_de}°C > 80°C - check lubrication & load"
        )
    
    if temp_nde > 80:
        recommendations.append(
            f"⚠️ Bearing NDE temperature {temp_nde}°C > 80°C - check lubrication & alignment"
        )
    
    if delta_temp > 15:
        recommendations.append(
            f"⚠️ Temperature difference {delta_temp}°C > 15°C - possible misalignment or unbalance"
        )
    
    if rise_de > 50 or rise_nde > 50:
        recommendations.append(
            "⚠️ Excessive temperature rise above ambient - check cooling & ventilation"
        )
